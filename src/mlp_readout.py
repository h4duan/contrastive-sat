...
val_dataloader = DataLoader(val_dataset, batch_size = args.batch_size, num_workers = args.num_workers, collate_fn = collate_fn_test, shuffle =  False)
...
if args.label_proportion != 0:
  train_dataset = SATInstances(args, start=test_size, data_length=train_size, labels=train_labels, n_vars=train_n_vars, augment = False)
  train_dataloader = DataLoader(train_dataset, batch_size = args.batch_size, num_workers = args.num_workers, collate_fn = collate_fn_test, shuffle = True)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

#supervised_train = supervised(args.dim)
class supervised_mlp(nn.Module):
    def __init__(self, in_dim):
        super(supervised_mlp, self).__init__()
        self.fc = MLP(in_dim, in_dim, 2)
...
supervised_train = supervised_mlp(args.dim)
supervised_train = supervised_train.cuda()
optim = optim.Adam(list(supervised_train.parameters()), lr=args.learning_rate, weight_decay=args.weight_decay)
best_acc = 0
...
for epoch in range(start_epoch, args.epochs):
  ...
  if epoch % args.test_epoch == 0:
    ...
    if accuracy > best_acc:
      ...
      if args.save_model:
        ...
        torch.save(supervised_train.state_dict(), os.path.join(args.model_dir, "mlp_" + str(jobid) + ".pth"))
  ...
  if args.label_proportion == 0:
    ...
    train_dataloader = DataLoader(train_dataset, shuffle=True, batch_size=args.batch_size,collate_fn=collate_fn_test, num_workers=args.num_workers)
